 ...
 * {@link org.apache.shiro.session.mgt.SessionValidationScheduler
 * SessionValidationScheduler} that uses Quartz to schedule a job to call
...
public class QuartzSessionValidationScheduler implements
	...
	public void enableSessionValidation() {
		...
		try {
					...
					.withSchedule(
							CalendarIntervalScheduleBuilder
									.calendarIntervalSchedule().withInterval(
											(int)sessionValidationInterval/1000,
											DateBuilder.IntervalUnit.SECOND)).startNow().build();
			// SimpleTrigger trigger = new Trigger(getClass().getName(),
			// Scheduler.DEFAULT_GROUP,
			// SimpleTrigger.REPEAT_INDEFINITELY,
			// sessionValidationInterval);

			JobDetail detail = JobBuilder.newJob()
					.ofType(QuartzSessionValidationJob.class)
			...
			detail.getJobDataMap().put(
					QuartzSessionValidationJob.SESSION_MANAGER_KEY,
					sessionManager);
	...
	public void disableSessionValidation() {
		...
		try {
			...
			if (log.isDebugEnabled()) {
				log.debug(
						"Could not cleanly remove SessionValidationJob from Quartz scheduler.  "
