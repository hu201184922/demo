...
Ace.util.jClass((function(){
    ...
    function encodeArguments(args){
        ...
        for (var i = 0, length = args.length; i < length; i++) {
            str = str.concat(Ace.encode(args[i])).concat(i < length - 1 ? ',' : '');
        }
        return str;
    }
    return {
        jClass: 'Ace.util.Logger',
        ...
        getTemplate: function(){
            if (!this.template) {
                this.template = new Ace.util.Template({
        ...
        isDebugEnabled: function(){
            return jQuery.browser.mozilla && Ace.util.Logger.isCategory('DEBUG', new Error('').stack.split('\n')[3].split('@')[1].replace(new RegExp('\S+'+request.getContextPath())).split(':')[0]);
        },
        ...
        debug: function(msg){
            if (this.isDebugEnabled()) {
                ...
                data.functionName = this.target.getFunctionName(Ace.util.Logger.prototype.debug.caller);
                ...
                data.msg = msg;
                data.args = encodeArguments(Ace.util.Logger.prototype.debug.caller.arguments);
        ...
        log: function(){
            ...
            for (var i = 0, len = arguments.length; i < len; i++) {
                var f = arguments[i];
                this.jClass.prototype[f] = Ace.proxy(this.jClass.prototype[f], function(i){
...
Ace.apply(Ace.util.Logger, {}, (function(){
    var library = null;
    var categorys = null;
    
    var getCategorys = function(){
        if (!categorys) {
            categorys = new Ace.util.Map();
    ...
    return {
        ...
        getLogger: function(jClass){
            ...
            var logger = new Ace.util.Logger(jClass);
            getLibrary().put(jClass, logger);
            return logger;
        }
    };
})());
Ace.Logger = Ace.util.Logger;
