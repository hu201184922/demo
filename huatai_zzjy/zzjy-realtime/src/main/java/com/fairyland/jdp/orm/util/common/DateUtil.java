...
public class DateUtil {
    ...
    private static SimpleDateFormat dateInputFmt = new SimpleDateFormat("yyyyMMdd", Locale.US);
    private static SimpleDateFormat dateOutputFmt = new SimpleDateFormat("yyyy-MM-dd", Locale.US);
	...
	return (big.getTime() - small.getTime()) / 86400000L;
    }

    public static int dayCount(Date big, Date small) {
	big = roundToDay(big);
	small = roundToDay(small);
	return new Double((big.getTime() - small.getTime()) / 86400000L).intValue();
    }

    public static double minuteInterval(Date big, Date small) {
	return (big.getTime() - small.getTime()) / 60000L;
    }

    public static double newMinuteInterval(Date big, Date small) {
	return (big.getTime() - small.getTime()) / 3600000L;
    }

    public static double intervalBySecond(Date big, Date small) {
	return (big.getTime() - small.getTime()) / 1000L;
    }
...
//	for (Date date : dates)
//	    log.debug(format(date, "yyyy-MM-dd"));
//    }
