<div id="${formId}-pages" class="pages megas512">
	<div class="pages_lf">
	<a href="<#if (page.currentPage lte 1) >#<#else>${(page.currentPage-1)?c}</#if>">«  上一页 </a><#--class="prev"-->
	<#assign pageIndexs = ObjectUtil.getPageArray(page,3)>
	<#list pageIndexs as index>
	<#if page.currentPage?c == index >
	...
	<a href="<#if page.currentPage gte page.totalPage >#<#else>${(page.currentPage+1)?c}</#if>"> 下一页 »</a>
	</div>
	<div class="pages_turn">
		<span class="fr" style="margin-top:4px;*margin-top:0px !important;">
			<input class="pages_go page-go" type="button"/>
		</span>
		<span class="fr">共${page.totalCount?c}条&nbsp;至  <input id="${formId}-page-go" type="text" class="pages_input"/> 页 </span>
	</div>
</div>
	...
	$(function(){
		...
		var rep = function(){
			...
			if($(this).val()!=''){
				...
				$(this).val(Fantasy.getMin(Fantasy.toInt($(this).val()),${page.totalPage?c}));
			}
		};
		$("#${formId}-page-go").bind({keyup:rep,afterpaste:rep,keydown:function(e){
			...
			if(e.keyCode == 40){
				...
				$(this).val(Fantasy.getMin(Fantasy.toInt($(this).val())+1,${page.totalPage?c}));
