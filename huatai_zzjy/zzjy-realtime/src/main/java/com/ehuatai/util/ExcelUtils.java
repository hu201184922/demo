...
public class ExcelUtils {
	...
	public static String getFileNameUrl(String fileName, HttpServletResponse response, Object[] headList,
			Object[] fieldList, List<Object> dataList) {
		String onlyFileName = "";
		...
		try {
			...
			if (null != dataList && !dataList.isEmpty()) {
				for (int n = 0; n < dataList.size(); n++) {
					// 在索引1的位置创建行（最顶端的行）
					XSSFRow row_value = sheet.createRow(n + 1);
					List<String> dataMap = (List<String>) dataList.get(n);
					for (int i = 0; i < dataMap.size(); i++) {
						XSSFCell cell = row_value.createCell(i);
						cell.setCellValue(dataMap.get(i).toString());
					}
				}
			}
			...
			CephUtil.uploadFile(servicefileUrl, "realdownloadfile", onlyFileName);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return onlyFileName;
