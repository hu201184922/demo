				...
				error: function(data){
					...
					var data={
						param:{
							...
							rows:[
								{contactName:'hello',contactTell:18616897921,contactAddress:'上海市宝山区富锦路2359号3号库',type:param.type,status:true,uid:'RW1404120001',number:Math.ceil(Math.random()*1000),time:window.GlOBAL.TOOL.deParam().time=='out'?new Date().getTime()-346573454:new Date().getTime()+346573454,name:'张毅',wearhouse:'上海宝钢物流有限公司三冠库一号库',pan:1},
								{contactName:'hello',contactTell:18616897921,contactAddress:'上海市宝山区富锦路2359号3号库',type:param.type,status:false,uid:'RW1404120301',number:Math.ceil(Math.random()*1000),time:window.GlOBAL.TOOL.deParam().time=='out'?new Date().getTime()-345345442:new Date().getTime()-34534544,name:'王五',wearhouse:'上海宝钢物流有限公司三冠库一号库',pan:2},
								{contactName:'hello',contactTell:18616897921,contactAddress:'上海市宝山区富锦路2359号3号库',type:param.type,status:true,uid:'RW1404125401',number:Math.ceil(Math.random()*1000),time:window.GlOBAL.TOOL.deParam().time=='out'?new Date().getTime()-5675732434:new Date().getTime()+5675732434,name:'张三',wearhouse:'上海宝钢物流有限公司三冠库一号库',pan:2},
								{contactName:'hello',contactTell:18616897921,contactAddress:'上海市宝山区富锦路2359号3号库',type:param.type,status:true,uid:'RW1404125601',number:Math.ceil(Math.random()*1000),time:window.GlOBAL.TOOL.deParam().time=='out'?new Date().getTime()-546565633:new Date().getTime()+546565633,name:'李四',wearhouse:'上海宝钢物流有限公司三冠库一号库',pan:1},
								{contactName:'hello',contactTell:18616897921,contactAddress:'上海市宝山区富锦路2359号3号库',type:param.type,status:false,uid:'RW1404120002',number:Math.ceil(Math.random()*1000),time:window.GlOBAL.TOOL.deParam().time=='out'?new Date().getTime()-5645343354:new Date().getTime()-5645343354,name:'谢力',wearhouse:'上海宝钢物流有限公司三冠库一号库',pan:2}
							]
						}
					};
					setTimeout(function(){
									...
									var _get=function(d,m){
										...
										for(var i=0;i<d.length;i++){
											var B={
												...
												1:{name:'待反馈',className:'yellow',stp:'<a class="ui-fk-list-head" href="'+PATH+'pandian-xq.html?id='+D[i].uid+'&type=1">',str:'<a class="ui-fk-list-btn1" href="'+PATH+'pandian-xq.html?id='+D[i].uid+'&type=1">去反馈</a>'},
												2:{name:'已完成',className:'grey',stp:'<a class="ui-fk-list-head" href="'+PATH+'pandian-xq.html?id='+D[i].uid+'&type=2">',str:'<a class="ui-fk-list-btn3" href="'+PATH+'pandian-xq.html?id='+D[i].uid+'&type=2">去查看</a>'}
											}[param.type],
											tip={1:'抽盘',2:'全盘'}[d[i].pan],
											C={
			...
			$.Bconfirm({
				...
				sureCallback:function(o,r,data){
					...
					$.Bstatus({
						...
						status:true,
						callback:function(){
							window.location=PATH+'pandian-xq.html?id='+id+'f&type=1';
						}
					});	
				}
			});
