...
UE.plugins['wordcount'] = function(){
    ...
    me.commands["wordcount"]={
        queryCommandValue:function(cmd,onlyCount){
           ...
           // contentText = this.getContentTxt().replace(reg,"");
            var sum=0;
            for (i = 0; i < this.getContentTxt().length; i++) {
            	
        		if ((this.getContentTxt().charCodeAt(i) >= 0) && (this.getContentTxt().charCodeAt(i) <= 255))
        			sum++;
        		else
        			sum += 2;
        	}
...
UE.plugins['customstyle'] = function() {
    ...
    me.setOpt({ 'customstyle':[
        {tag:'h1', label:'居中标题', style:'font-size:24px;font-weight:bold;line-height:38px;border-bottom:#ccc 2px solid;padding:0 4px 0 0;text-align:center;margin:0 0 20px 0;'},
        {tag:'h1', label:'居左标题', style:'font-size:24px;font-weight:bold;line-height:38px;border-bottom:#ccc 2px solid;padding:0 4px 0 0;text-align:left;margin:0 0 10px 0;'},
