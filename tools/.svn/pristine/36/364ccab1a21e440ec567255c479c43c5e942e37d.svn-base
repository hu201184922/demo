...
$(document).ready(function(){
	...
	function addHoverDom(treeId, treeNode) {
		...
		if (btn)
			btn.bind("click", function() {
				$.post("${ctx}/admin/dicts/getDictItem/" + treeNode.id + "",
						function(data) {
							if (data == "") {
								create(treeNode);
								return true;
	...
	function beforeDrop(treeId, treeNodes, targetNode, moveType, isCopy) {
		...
		if (treeNodes[0].pid != parentId) {
			$.ajax({
				type : "POST",
				url : "${ctx}/admin/dicts/move/" + treeNodes[0].id,
				data : {
					"parentId" : parentId
				},
				success : function(msg) {
	...
	$(document).ready(function() {
		...
		$.getJSON("${ctx}/admin/dicts/get-tree?" + new Date(), function(data) {
			var zNodes = data;
	...
	function createDictionItemPage() {
		...
		location.href = "${ctx}/admin/dicts/createDictItempage/" + id + "";
	};
	...
	function beforeLoad() {
		...
		this.href = "${ctx}/admin/dicts/createDictItempage/" + id + "";
		return true;
	};
	$(document).ready(function() {
		$(".b").fancybox({
