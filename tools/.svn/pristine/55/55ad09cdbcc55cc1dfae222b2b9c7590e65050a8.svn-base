...
function userModu(){
    $(function () {
	    ...
	    $(function () {
	        ...
	        $.getJSON('${pageContext.request.contextPath}/admin/userBehInfo/getVersionList',{type:'0'},function(data){
	        	...
	        	$.each(data,function(i,v){
	        		strVersionCode+="<span style='width: 70px;text-align: center;display: inline-block;border: 0.1px #ccc solid;float: left;'>"+v.versionCode+"</span>";
	        		strVersionId+="<span style='width: 70px;text-align: center;display: inline-block;border: 0.1px #ccc solid;float: left;'>"+v.versionId+"</span>";
	        	})
	...
	$('#targetAjaxForm').ajaxForm(function(data) {
		...
		$("#minalterType").html(data.minalterType+"<font>PC</font>")
