...
public class NumberUtil {
    ...
    static String[] numberChinese = new String[] { "零", "壹", "贰", "叁", "肆", "伍", "陆", "柒", "捌", "玖" };
    static String[] unitChinese = new String[] { "元", "拾", "佰", "仟", "万", "亿", "角", "分", "整" };
	...
	chinese = chinese.length() > 1 ? toRMB(chinese.substring(0, chinese.length() - 1), unit + 1) + (numberChinese[Integer.valueOf(chinese.substring(chinese.length() - 1))] + unitChinese[index]) : (numberChinese[Integer.valueOf(chinese)] + unitChinese[index]);
...
//	log.debug(toRMB("10000100000.10"));
//	log.debug(toRMB("1111111111.11"));
//
//	log.debug(toRMB("11010001000.11"));
//    }
}