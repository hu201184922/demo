...
public interface BaseMapper {
	...
	@Select("SELECT TEAMCOMSHORTNAME AS SUBNAME, MANAGECOMNAME AS NAME, MANAGECOMCODE AS CODE FROM D_MANAGECOM WHERE LENGTH (MANAGECOMCODE) = 1")
	List<HashMap<String, String>> getAllManage();

	@Select("SELECT * FROM ( SELECT TEAMCOMSHORTNAME AS SUBNAME, MANAGECOMNAME AS NAME, MANAGECOMCODE AS CODE FROM D_MANAGECOM WHERE LENGTH(MANAGECOMCODE) = 1 UNION SELECT DISTINCT TEAMCOMSHORTNAME AS SUBNAME, PROVCOMNAME AS NAME, PROVCOMCODE AS CODE FROM D_MANAGECOM WHERE LENGTH(PROVCOMCODE) = 3 ) ORDER BY LENGTH(CODE),CODE")
	List<HashMap<String, String>> getAllManageAndCompany();

	@Select("SELECT * FROM ( SELECT TEAMCOMSHORTNAME AS SUBNAME, MANAGECOMNAME AS NAME, MANAGECOMCODE AS CODE FROM D_MANAGECOM WHERE LENGTH(MANAGECOMCODE) = 1 UNION SELECT DISTINCT TEAMCOMSHORTNAME AS SUBNAME, PROVCOMNAME AS NAME, PROVCOMCODE AS CODE FROM D_MANAGECOM WHERE LENGTH(PROVCOMCODE) = 3 UNION SELECT DISTINCT ACTUTEAMCOMSHORTNAME AS SUBNAME, COUNTCOMNAME AS NAME, COUNTCOMCODE AS CODE FROM D_MANAGECOM WHERE LENGTH(COUNTCOMCODE) = 5) ORDER BY LENGTH(CODE),CODE")
	List<HashMap<String, String>> getAllManageAndCompanyAndBranch();

	@Select("SELECT DISTINCT TEAMCOMNAME AS SUBNAME, TEAMCOMNAME AS NAME, TEAMCOMCODE AS CODE FROM D_ORGANIZATION WHERE	TEAMCOMCODE IS NOT NULL AND TEAMCOMCODE <> COUNTCOMCODE AND STATUS='1' ORDER BY CODE")
	List<HashMap<String, String>> getAllServerOrg();

	@Select("SELECT DISTINCT COST_CENTERDEVNAME AS SUBNAME,	COST_CENTERDEVNAME AS NAME,	COST_CENTERSTDCODE AS CODE FROM	D_ORGANIZATION WHERE COST_CENTERSTDCODE IS NOT NULL AND LENGTH(COST_CENTERSTDCODE)=10 AND COST_CENTERSTDCODE <> COUNTCOMCODE AND STATUS='1' ORDER BY CODE")
	List<HashMap<String, String>> getAllCenterOrg();
