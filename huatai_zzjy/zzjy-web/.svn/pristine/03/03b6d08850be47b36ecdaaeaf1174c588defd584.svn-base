...
function fileQueued(file) {
	try {
		...
		progress.setStatus("<font style='font-size:12px;'>请点击上传</font>");
...
function uploadStart(file) {
	try {
		...
		var defaultPath = href.substring(0, href.indexOf("/isp", 1));
		progress.setStatus("<font style='font-size:12px;'>正在上传</font> <img src='"+window.contextPath+"/static/images/loading_16.gif' />");
		progress.toggleCancel(true, this);
	}
	catch (ex) {}
...
function uploadSuccess(file, serverData) {
	try {
		...
		$('#fsUploadProgress').view().add(json);
	} catch (ex) {
		this.debug(ex);
	}
}
