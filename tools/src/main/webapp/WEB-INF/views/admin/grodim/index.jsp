...
<script type="text/javascript">
	$(function() {
			$('#pager').pager('${pageContext.request.contextPath}/admin/grodim/list',15,$('#templetView').view().on('add',function(data){
				(function(zhis){
					...
					deleteConfirm.on('confirm',function(w, t) {
						$.getJSON('${pageContext.request.contextPath}/admin/grodim/delete',{id:data.gdId},function(bit){
							if(bit==true){
					...
					zhis.target.find('.save').click(function(){
						$.getJSON('${pageContext.request.contextPath}/admin/grodim/update',zhis.getData(),function(data){
		...
		$("#createTemplet").click(function() {
			...
			$('input[name="groDimTypeCode"]').val("");
			$('input[name="groDimTypeName"]').val("");
		});
	...
	function query(){
		var postData ={
				"groDimTypeCode":$("#groDimTypeCodeQuery").val(),
				"groDimTypeName":$("#groDimTypeNameQuery").val()
		}
		$('#pager').pager().setPostData(postData);
		$('#pager').pager().reload();
	}
	function sub(){
		var groDimTypeCode = $('input[name="groDimTypeCode"]').val()
		var groDimTypeName = $('input[name="groDimTypeName"]').val();
		if ($.trim(groDimTypeCode) == '') {
			$.dialog({time: 2000,content:  "请输入分组维度分类编码！"});
			return;
		}
		if ($.trim(groDimTypeName) == '') {
			$.dialog({time: 2000,content:  "请输入分组维度分类名称！"});
			return;
		}
...
<body>
		 	...
		 	<div class="business_search">
				<div class="business_search_left" style="margin-left: -20px;">
					<form id="templetAjaxForm"
						action="${pageContext.request.contextPath}/admin/grodim/list"
						method="post">
						...
						<table>
							<tbody>
								<tr>
									...
									<td style="text-align: left;">分组维度分类编码：</td>
									<td><input style="width: 200px;height: 30px;border-radius: 5px;border:1px solid #90C1E6;" type="text" name="groDimTypeCodeQuery" id="groDimTypeCodeQuery"  class="text">&nbsp;&nbsp;&nbsp;</td>

									<td style="text-align: left;">分组维度分类名称：</td>
									
									
									
									<td><input style="width: 200px;height: 30px;border-radius: 5px;border:1px solid #90C1E6;" type="text" name="groDimTypeNameQuery" id="groDimTypeNameQuery" class="text">&nbsp;&nbsp;&nbsp;</td>
			...
			<table cellspacing="0" cellpadding="0" class="t-list table"id="templetView">
				<tbody>
					...
					<tr class="tb_tr_content template" name="default">
					    <td>{groDimTypeCode}</td>
					    <td>{groDimTypeName}</td>
						<td>{createTime}</td>
						<td>{modifyTime}</td>
                        <td style="border-right: 1px solid #ccc;width:40px"><span class="span_edit">
						
					
							<a href="${pageContext.request.contextPath}/admin/groDimDetail/index?gdId={gdId}" style="float: left;margin-right:5px">
					 ...
					 <tr class="tb_tr_content template" name="update">
					 	<td><input style="width: 100px;"
							class="text_wd180 view-field" name="groDimTypeCode" type="text"
							 /></td>
						<td><input style="width: 100px;"
							class="text_wd180 view-field" name="groDimTypeName" type="text"
							 /></td>
						<td>{createTime}</td>
						<td>{modifyTime}</td>
	...
	<form id="saveTempletForm" style="display: none;"
		  action="${pageContext.request.contextPath}/admin/grodim/create"
		  method="post">
		<div id="tagContent2" class="tagContent tagContent_div"
			...
			<table style="line-height: 30px">
				<tbody>
					<tr>
						<td class="text_right">分组维度分类编码：</td>
						<td class="text_left"><input type="text" name="groDimTypeCode" style="border-radius:5px; width:200px;height: 30px;border:1px solid #90C1E6;"/></td>
					</tr>
					<tr><td height="30px" colspan="2"></td></tr>
					<tr>
						<td class="text_right">分组维度分类名称：</td>
						<td class="text_left"><input type="text" name="groDimTypeName" style="border-radius:5px; width:200px;height: 30px;border:1px solid #90C1E6;"/></td>
					</tr>
					<tr><td height="30px" colspan="2"></td></tr>
					<tr>
						<td></td>
